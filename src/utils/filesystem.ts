...
export function createEntityDirectories(entityName: string, projectPath?: string): Record<string, string> {
  ...
  try {
    ...
    const appSubdirs = ['controller', 'documentation'];
...
export async function renderTemplate(
  ...
  try {
    ...
    throw new Error(`Failed to render template ${templateFile}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function createNestJSFiles(projectPath: string, projectName: string): Promise<void> {
  ...
  try {
    ...
    const diFiles = [
      ...
      { template: 'application/di/infrastructure/DatabaseModule.ts.ejs', dest: 'src/application/di/infrastructure/DatabaseModule.ts' }
    ];
    ...
    const apiFiles = [
      { template: 'application/api/rest/common/filter/RestApiHttpExceptionFilter.ts.ejs', dest: 'src/application/api/rest/common/filter/RestApiHttpExceptionFilter.ts' },
      { template: 'application/api/rest/common/interceptor/RestApiResponseInterceptor.ts.ejs', dest: 'src/application/api/rest/common/interceptor/RestApiResponseInterceptor.ts' },
      { template: 'application/api/rest/common/query/RestApiGetEntitiesQuery.ts.ejs', dest: 'src/application/api/rest/common/query/RestApiGetEntitiesQuery.ts' },
      { template: 'application/api/rest/common/query/index.ts.ejs', dest: 'src/application/api/rest/common/query/index.ts' }
    ];
